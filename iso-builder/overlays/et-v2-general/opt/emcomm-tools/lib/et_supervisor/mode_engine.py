...
class ModeEngine:
    ...
    def _run_action(self, action, params=None):
        ...
        elif action_type == "vara-kiss-config":
            self._apply_vara_kiss_config(
                action.get("modem", "hf"),
                bool(action.get("enable", False)))
        elif action_type == "vara-kiss-port":
            # Runtime params (from an automation mission's kiss_port field)
            # override the mode JSON's hardcoded value. Lets each mission
            # pick a per-station KISS port from the Automation Editor UI
            # without editing the mode JSON.
            port_override = (params or {}).get("kiss_port")
            port = int(port_override) if port_override else int(action.get("port", 8100))
            self._apply_vara_kiss_port(action.get("modem", "hf"), port)
    ...
    def _run_kill_all(self):
        ...
        self._kill_wineserver()
        # Also kill any leftover LinBPQ instance. bbs-server spawns linbpq
        # inside a QtTerminal wrapper; closing the terminal doesn't always
        # take the child linbpq down with it, and a survivor holds port
        # 8010 (telnet) + 8012 (HTTP).
        # `-x linbpq` matches ONLY processes named exactly `linbpq` —
        # NOT `QtTerminal --title=LinBPQ ... -e linbpq` (whose argv
        # contains "linbpq"). Earlier `-f linbpq` also killed QtTerminal,
        # which is the process supervisor tracks — supervisor lost the
        # reference and mode restart broke.
        try:
            subprocess.run(
                ["pkill", "-x", "linbpq"],
    ...
    def _apply_mercury_config(self):
        ...
        # Radio model — pull the Hamlib numeric id from the operator's
        # selected radio (conf/radios.d/active-radio.json → rigctrl.id).
        # Historically hardcoded to "2" (NET rigctl) which happened to work
        # on some Yaesus (FT-897 / FT-857) by coincidence but broke on
        # anything Mercury talks to directly (e.g. FT-891): Mercury spoke
        # NET-rigctl protocol at a rigctld backend that expected the
        # specific rig's CAT commands, PTT never keyed, connect hung.
        # Falls back to "2" only if active-radio.json is missing or malformed.
        radio_model = "2"
        active_radio_path = os.path.join(ET_HOME,
                                          "conf/radios.d/active-radio.json")
        try:
            with open(active_radio_path) as _f:
                _rd = json.load(_f)
            _id = (_rd.get("rigctrl") or {}).get("id")
            if _id:
                radio_model = str(_id).strip()
                log.info("Mercury radio_model from active-radio.json: %s (%s %s)",
                         radio_model, _rd.get("vendor", "?"), _rd.get("model", "?"))
        except (FileNotFoundError, json.JSONDecodeError, KeyError) as _e:
            log.warning("Could not read radio model from %s (%s) — "
                        "falling back to radio_model=2 (NET rigctl)",
                        active_radio_path, _e)
    ...
    def _restart_rigctld(self):
        """Restart rigctld via the authoritative recovery script.

        li-rigctld-recover does the full dance (reset-failed, stop,
        killall, wait for port free, start, verify) and returns 0 only
        when rigctld is up AND the rig responds to a 'get freq' probe.
        Central single source of truth — same script called by both
        this path and the li-automation daemon's rigctld_ensure_ready.
        """
        log.info("Running li-rigctld-recover...")
        rig.close()  # drop stale rig client TCP connection first
        try:
            r = subprocess.run(
                ["/opt/liaisonos/bin/li-rigctld-recover", "--quiet"],
                timeout=30, capture_output=True, text=True
            )
            if r.returncode == 0:
                log.info("rigctld recovered")
            else:
                log.warning("li-rigctld-recover exit %d: %s",
                            r.returncode, r.stderr.strip()[:400])
        except (subprocess.TimeoutExpired, FileNotFoundError) as e:
            log.warning("li-rigctld-recover failed: %s", e)
    ...
    def _apply_vara_kiss_port(self, modem, port):
        # Change VARA's KISS listen port so LinBPQ (hardcoded for the
        # default 8100 in bpq32.cfg) can't attach during a Winlink
        # mission — while leaving KISS itself ENABLED so PAT (which
        # uses ARQ, not KISS) is unaffected either way. VARA reads this
        # value at startup; ordering (pre_start action → VARA.exe launch
        # by the mode) makes it stick without a restart.
        ini_paths = {
            "hf": os.path.expanduser("~/.wine32/drive_c/VARA/VARA.ini"),
            "fm": os.path.expanduser("~/.wine32/drive_c/VARA FM/VARAFM.ini"),
        }
        ini_path = ini_paths.get(modem)
        if not ini_path:
            log.warning("vara-kiss-port: unknown modem '%s'", modem)
            return
        if not os.path.isfile(ini_path):
            log.warning("vara-kiss-port: %s not found, skipping", ini_path)
            return
        self._update_ini_keys(ini_path, {"KISS Port": str(port)})
        log.info("VARA %s KISS Port -> %d (%s)",
                 modem.upper(), port, ini_path)

    def _apply_vara_kiss_config(self, modem, enable):
        # Presence modes need KISS on so LinBPQ can attach; Winlink ARQ
        # missions need it off so PAT can grab the modem. VARA reads this
        # once at startup, so the ordering (pre_start action → VARA.exe
        # launched by the mode) is what makes it stick without a restart.
        ini_paths = {
            "hf": os.path.expanduser("~/.wine32/drive_c/VARA/VARA.ini"),
            "fm": os.path.expanduser("~/.wine32/drive_c/VARA FM/VARAFM.ini"),
        }
        ini_path = ini_paths.get(modem)
        if not ini_path:
            log.warning("vara-kiss-config: unknown modem '%s'", modem)
            return
        if not os.path.isfile(ini_path):
            log.warning("vara-kiss-config: %s not found, skipping", ini_path)
            return
        self._update_ini_keys(ini_path, {"Enable KISS": "1" if enable else "0"})
        log.info("VARA %s KISS -> %s (%s)",
                 modem.upper(), "1" if enable else "0", ini_path)
    ...
    @staticmethod
    def _validate_bpq32_content(content):
        """Return list of hard-error strings; empty list == cfg is safe.

        Catches the corruption patterns that make LinBPQ exit-code-0-
        with-no-output on startup — paste damage, BOM, unbalanced blocks.
        Anything that would prevent LinBPQ from parsing the file at all
        goes here. Style/preference nits are NOT flagged (would cause
        false positives on hand-tuned patched-LinBPQ setups).
        """
        import re
        errors = []
        # Stray control characters — the classic "nano paste through
        # a weird clipboard" corruption. Excludes TAB (0x09), LF (0x0a),
        # CR (0x0d) which are legal in a text file.
        ctrl = re.findall(r'[\x00-\x08\x0b\x0c\x0e-\x1f]', content)
        if ctrl:
            errors.append(f"{len(ctrl)} stray control character(s) "
                          "(paste corruption?)")
        # UTF-8 BOM — invisible in editors, breaks LinBPQ's parser.
        if content.startswith('﻿'):
            errors.append("UTF-8 BOM at start of file")
        # PORT / ENDPORT balance. If unbalanced, LinBPQ often bails
        # silently mid-parse and exits code 0 with no useful log.
        ports = 0
        endports = 0
        for line in content.split('\n'):
            if re.match(r'^\s*PORT\b',    line, re.IGNORECASE): ports += 1
            if re.match(r'^\s*ENDPORT\b', line, re.IGNORECASE): endports += 1
        if ports != endports:
            errors.append(f"unbalanced PORT/ENDPORT ({ports} vs {endports})")
        return errors

    def _apply_bbs_config(self, template_name):
        """Template BBS LinBPQ config with callsign, grid, password.

        Renders the template in memory, validates the rendered output,
        and ONLY writes to disk if validation passes. If validation
        fails, the existing runtime bpq32.cfg is left untouched — LinBPQ
        starts with the last-known-good copy, and the operator is
        notified so they can fix the template.
        """
        if not template_name:
        ...
        # Render in memory FIRST — no filesystem write until we've
        # confirmed the rendered content is sane. This is the whole
        # point of the guard: never clobber a working bpq32.cfg with
        # a broken render.
        ...
        errors = self._validate_bpq32_content(content)
        if errors:
            log.error("Rendered BBS config failed validation "
                      "(template=%s) — keeping existing %s",
                      template_name, target_path)
            for e in errors:
                log.error("  - %s", e)
            self._notify_user(
                f"⚠ BBS template '{template_name}' has errors — "
                "kept existing bpq32.cfg. Check supervisor log.")
            return  # do NOT overwrite target
